...
public sealed class GetVesselDetailBffQueryHandler
    : AizenQueryHandler<GetVesselDetailBffQuery, AdminVesselDetailBffResponse>
{
    private readonly IVesselRemoteCall _vessel;
    private readonly IServiceRequestRemoteCall _serviceRequest;
    private readonly ICargoDryRemoteCall _cargoDry;
    private readonly ILogger<GetVesselDetailBffQueryHandler> _logger;

    public GetVesselDetailBffQueryHandler(
        IVesselRemoteCall vessel,
        IServiceRequestRemoteCall serviceRequest,
        ICargoDryRemoteCall cargoDry,
        ILogger<GetVesselDetailBffQueryHandler> logger)
    {
        _vessel = vessel;
    ...
    public override async Task<AdminVesselDetailBffResponse?> Handle(
        GetVesselDetailBffQuery request, CancellationToken cancellationToken)
    {
...
vesselId: request.VesselId, pageIndex: 0, pageSize: 10);
        ...
        try
        {
            var kitsResult = await _cargoDry.GetKitsAsync(status: null, search: null, vesselId: request.VesselId, ownerUserId: null, batchCode: null, page: 0, pageSize: 100, ct: cancellationToken);
