...
/// marine-mobile-bff confidential client. Used for BFF → module calls (Authorization header)
/// and for Keycloak Admin API calls. Never logs the secret or the token.
/// </summary>
public interface IParticipantKeycloakServiceTokenProvider
{
    Task<string> GetAccessTokenAsync(CancellationToken cancellationToken = default);
}

internal sealed class ParticipantKeycloakServiceTokenProvider : IParticipantKeycloakServiceTokenProvider
{
    private const string CacheKey = "marine-mobile-bff:keycloak-service-token";

    private readonly IHttpClientFactory _httpClientFactory;
    private readonly IMemoryCache _cache;
    private readonly MarineMobileKeycloakOptions _options;

    public ParticipantKeycloakServiceTokenProvider(
