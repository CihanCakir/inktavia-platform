...
/// <summary>
/// Fail-envelope fidelity handler for Provider BFF → module calls (port of the AdminPanel BFF's
/// <c>AdminBffFailEnvelopeHandler</c> — FIX_PROVIDER_ENVELOPE_FIDELITY).
///
/// A module surfaces business errors (e.g. CargoDry's <c>SR_STOCK_REQUEST_DUPLICATE_PENDING</c> or Payment's
/// <c>ProviderNotSplitEligible</c> = 5091) as an <c>AizenBusinessException</c> which its
...
/// 4xx/5xx and throws a <c>Refit.ApiException</c>; that is NOT an <c>AizenBusinessException</c>, so the provider
/// BFF's own global middleware maps it to a generic error (code 911) and the module's
/// <c>errorCode</c>/<c>errorMessage</c> are lost — the FE then shows a raw HTTP error (e.g. SR_STOCK_REQUEST_DUPLICATE_PENDING lost).
///
...
/// so the provider middleware returns a structured envelope the app's <c>normalizeEnvelope</c> maps to
...
/// since every provider-web surface benefits from verbatim module error codes. It does NOT touch
/// <c>Core.RemoteCall</c> internals or the global exception middleware.
/// </summary>
public sealed class ProviderBffFailEnvelopeHandler : DelegatingHandler
{
