...
public sealed class ArchiveContentItemCommandHandler
    : AizenCommandHandler<ArchiveContentItemCommand, ContentItemDto>
{
    ...
    private readonly ILogger<ArchiveContentItemCommandHandler> _logger;

    public ArchiveContentItemCommandHandler(
        IContentItemRepository items,
        IAizenInfoAccessor info,
        IAizenMessagePublisher publisher,
        IContentCacheInvalidator cache,
        ILogger<ArchiveContentItemCommandHandler> logger)
    {
        _items = items;
        _info = info;
    ...
    public override async Task<ContentItemDto?> Handle(
        ArchiveContentItemCommand request, CancellationToken cancellationToken)
    {
        ...
        ContentAuthorization.EnsureElevated(_info, "Archive content");
        ContentStatusTransition.EnsureCanArchive(item.Status);
        ...
        // Archiving a live (published) item removes it from public surfaces — treat as an unpublish edge.
        if (wasPublished)
        {
            ...
            catch (Exception ex)
            {
                _logger.LogWarning(ex,
                    "Failed to publish ContentUnpublishedMessage on archive for ContentId={Id}.", item.Id);
            }
