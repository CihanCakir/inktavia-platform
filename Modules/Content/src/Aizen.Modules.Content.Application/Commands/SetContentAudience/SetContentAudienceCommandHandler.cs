...
public sealed class SetContentAudienceCommandHandler
    : AizenCommandHandler<SetContentAudienceCommand, ContentItemDto>
{
    ...
    public SetContentAudienceCommandHandler(IContentItemRepository items, IContentCacheInvalidator cache)
    {
        _items = items;
        _cache = cache;
    }
    ...
    public override async Task<ContentItemDto?> Handle(
        SetContentAudienceCommand request, CancellationToken cancellationToken)
    {
        ...
        item.Audience = ContentMapper.ToVo(request.Audience);
