...
[DocumentationInfo("Link file to owner command handler", "Resolves the file by Guid, delegates to IFileOwnershipService to link the file, then publishes FileLinkedToOwnerMessage.")]
public sealed class LinkFileToOwnerCommandHandler : AizenCommandHandler<LinkFileToOwnerCommand, FileOwnerReferenceDto>
{
    ...
    public LinkFileToOwnerCommandHandler(
        IFileOwnershipService ownershipService,
    ...
    public override async Task<FileOwnerReferenceDto?> Handle(LinkFileToOwnerCommand command, CancellationToken cancellationToken)
    {
        ...
        var result = await _ownershipService.LinkFileToOwnerAsync(
            file.Id, command.Request, userId, cancellationToken);
