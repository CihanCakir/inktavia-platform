...
/// <summary>
/// Resolves an Organizer profile by Keycloak subject and maps it to the provider-safe detail DTO.
/// Data access goes through <see cref="IUserProfileRepository"/>; no CQRS-in-CQRS dispatch. Null when unlinked.
/// </summary>
public sealed class GetOrganizerProfileByKeycloakSubjectQueryHandler
    : AizenQueryHandler<GetOrganizerProfileByKeycloakSubjectQuery, OrganizerProfileDetailDto>
{
    private readonly IUserProfileRepository _profiles;

    public GetOrganizerProfileByKeycloakSubjectQueryHandler(IUserProfileRepository profiles)
    {
        _profiles = profiles;
    }

    public override async Task<OrganizerProfileDetailDto?> Handle(
        GetOrganizerProfileByKeycloakSubjectQuery request, CancellationToken cancellationToken)
    {
        var profile = await _profiles.GetOrganizerProfileByKeycloakSubjectAsync(request.KeycloakSubject, cancellationToken);
        if (profile is null)
            return null;
        ...
        return new OrganizerProfileDetailDto
        {
            ...
            BusinessAddressLabel = profile.BusinessAddressLabel,
