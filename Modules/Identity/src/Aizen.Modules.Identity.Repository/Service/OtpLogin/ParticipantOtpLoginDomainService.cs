...
public sealed class ParticipantOtpLoginDomainService : IParticipantOtpLoginDomainService
{
    ...
    private readonly ILogger<ParticipantOtpLoginDomainService> _logger;

    private const string RateLimitKeyPrefix = "participant:otplogin:rl:";

    public ParticipantOtpLoginDomainService(
        ...
        ILogger<ParticipantOtpLoginDomainService> logger)
    {
    ...
    public async Task<OtpLoginRequestResult> RequestAsync(string channel, string identifier, CancellationToken ct)
    {
        ...
        _db.ParticipantOtpLoginRequests.Add(entity);
    ...
    public async Task<OtpLoginVerifyResult> VerifyOtpAsync(string loginRequestId, string otpCode, CancellationToken ct)
    {
        ...
        var record = await _db.ParticipantOtpLoginRequests
        ...
        {
            _db.ParticipantOtpLoginRequests.Remove(record);
            await _db.SaveChangesAsync(ct);
            return invalid;
        }
        ...
        try
        {
            var ticket = await _ticketService.MintAsync(record.KeycloakSubjectId, "inktavia-mobile", ct);
