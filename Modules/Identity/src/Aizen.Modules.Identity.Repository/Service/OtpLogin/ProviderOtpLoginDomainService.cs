...
public sealed class ProviderOtpLoginDomainService : IProviderOtpLoginDomainService
{
    ...
    private readonly ILogger<ProviderOtpLoginDomainService> _logger;

    private const string RateLimitKeyPrefix = "provider:otplogin:rl:";

    public ProviderOtpLoginDomainService(
        ...
        ILogger<ProviderOtpLoginDomainService> logger)
    {
    ...
    public async Task<OtpLoginRequestResult> RequestAsync(string channel, string identifier, CancellationToken ct)
    {
        ...
        var entity = ProviderOtpLoginRequestEntity.Create(
            loginRequestId: syntheticResult.LoginRequestId,
            keycloakSubjectId: user.KeycloakSubjectId,
            userId: user.Id, providerProfileId: profile.Id,
        ...
        _db.ProviderOtpLoginRequests.Add(entity);
    ...
    public async Task<OtpLoginVerifyResult> VerifyOtpAsync(string loginRequestId, string otpCode, CancellationToken ct)
    {
        ...
        var record = await _db.ProviderOtpLoginRequests
        ...
        {
            _db.ProviderOtpLoginRequests.Remove(record);
            await _db.SaveChangesAsync(ct);
            return invalid;
        }
        ...
        try
        {
            var ticket = await _ticketService.MintAsync(record.KeycloakSubjectId, "provider-portal", ct);
    ...
    public async Task<OtpLoginResendResult> ResendAsync(string loginRequestId, CancellationToken ct)
    {
        ...
        {
            // Still cooling down: no new OTP is generated and nothing is dispatched, so do NOT claim
            // it was resent — the UI would otherwise reset its timer and tell the user a fresh code
            // is on the way while the previous code is still the only valid one.
