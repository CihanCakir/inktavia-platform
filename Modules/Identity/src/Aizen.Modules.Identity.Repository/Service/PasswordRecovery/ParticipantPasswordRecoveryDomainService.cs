...
/// <summary>
/// Participant (mobile) password recovery. A byte-for-byte mirror of
/// <see cref="ProviderPasswordRecoveryDomainService"/> — the only deltas are the profile gate
/// (<see cref="WorkshopRoleContext.Participant"/>), the persisted entity
/// (<see cref="ParticipantPasswordRecoveryRequestEntity"/> → <c>participant_password_recovery_requests</c>),
/// and the per-identifier rate-limit key prefix. The shared crypto (<see cref="PasswordRecoverySecurity"/>),
/// options (<see cref="PasswordRecoveryOptions"/>), Keycloak password service, notifier, and result models
/// are reused unchanged.
/// </summary>
public sealed class ParticipantPasswordRecoveryDomainService : IParticipantPasswordRecoveryDomainService
{
    ...
    private readonly ILogger<ParticipantPasswordRecoveryDomainService> _logger;

    private const string RateLimitKeyPrefix = "participant:pwreset:rl:";

    public ParticipantPasswordRecoveryDomainService(
        ...
        ILogger<ParticipantPasswordRecoveryDomainService> logger)
    {
        _db = db;
    ...
    public async Task<PasswordRecoveryRequestResult> RequestAsync(string channel, string identifier, CancellationToken ct)
    {
        ...
        var entity = ParticipantPasswordRecoveryRequestEntity.Create(
        ...
        _db.ParticipantPasswordRecoveryRequests.Add(entity);
    ...
    public async Task<PasswordRecoveryVerifyResult> VerifyOtpAsync(string resetRequestId, string otpCode, CancellationToken ct)
    {
        ...
        var record = await _db.ParticipantPasswordRecoveryRequests
        ...
        {
            _db.ParticipantPasswordRecoveryRequests.Remove(record);
            await _db.SaveChangesAsync(ct);
            return invalid;
        }
