...
{
    public sealed class VenueRegistrationDomainService : IVenueRegistrationDomainService
    {
        ...
        public VenueRegistrationDomainService(
        ...
        public async Task<(UserEntity user, UserProfileEntity profile)> RegisterOrAttachAsync(
            RegisterVenueDomainModel m, CancellationToken ct)
        {
            if (!m.KvkkAccepted)
            ...
            // rol ata
            var role = await _roleManager.FindByNameAsync(RoleNames.Venue)
