...
[DocumentationInfo("Reject assignment command handler", "Provider rejects the assignment and publishes AssignmentRejected event.")]
public sealed class RejectServiceRequestAssignmentCommandHandler : AizenCommandHandler<RejectServiceRequestAssignmentCommand, RejectServiceRequestAssignmentResponse>
{
    ...
    public RejectServiceRequestAssignmentCommandHandler(
    ...
    public override async Task<RejectServiceRequestAssignmentResponse?> Handle(RejectServiceRequestAssignmentCommand request, CancellationToken cancellationToken)
    {
        ...
        await _realtimePublisher.PublishAsync(sr.Id, sr.RequestCode, sr.OwnerUserId, assignment.ProviderProfileId,
            ServiceRequestRealtimeEventType.AssignmentRejected, assignment.ToDto(),
            currentUserId, ServiceRequestActorType.Provider, cancellationToken);

        return new RejectServiceRequestAssignmentResponse(assignment.Id);
    }
}
