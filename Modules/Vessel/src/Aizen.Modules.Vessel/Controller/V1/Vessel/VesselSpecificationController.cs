...
public sealed class VesselSpecificationController : AizenWebApiController
{
    ...
    {
        var result = await _cqrs.ProcessAsync<VesselSpecificationDto>(new UpsertVesselSpecificationCommand(vesselId, req, CurrentUserId), ct);
        return SetResponse(result);
    }
    ...
    {
        await _cqrs.ProcessAsync<bool>(new RemoveVesselSpecificationCommand(vesselId, CurrentUserId), ct);
